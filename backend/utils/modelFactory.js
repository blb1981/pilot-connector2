...
exports.getAllSoft = async (Model, req, res, resourceName) => {
	try {
		...
		apiResponse(res, 200, `List of all resource - ${resourceName}, including soft deleted`, results)
	} catch (error) {
		errorResponse(res, error)
	}
}
...
// Get count
exports.getCount = async (Model, req, res, resourceName) => {
	try {
		const count = await Model.findAll()
		apiResponse(res, 200, `Total number of resource - ${resourceName}`, { count: count.length })
	} catch (error) {
		errorResponse(res, error)
	}
}
